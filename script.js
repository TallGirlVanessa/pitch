{
      ...
      if (recording && !paused) {
        ...
        for (var i = 0; i < buf.length; i++) {
          ...
          if (analpos == anallen) {
            ...
            if (fxest.en > 0) {
              if (fxest.en < enmin) enmin = fxest.en;
              else enmin = 0.99 * enmin + 0.01 * fxest.en;
              if (fxest.en > enmax) enmax = fxest.en;
              else enmax = 0.99 * enmax + 0.01 * fxest.en;
            }

            if (fxest.en > 0.1 * enmax)
              track.push([fxest.fx, fxest.en / enmax, fxest.vs]);
            else track.push([0, 0, 0]);
            ...
            for (analpos = 0, j = anallen / 5; j < anallen; j++, analpos++)
              analbuf[analpos] = analbuf[j];
          }
        }
      }
    };
  ...
  function doRun() {
    if (context == null) {
      ...
      try {
        ...
        alert(
          "Web Audio API is not supported in this browser. Try Chrome, Firefox or Safari."
        );
      }
    }
    ...
    if (micsource == null) {
      ...
      navigator.getMedia(
        ...
        startrecording,
        function () {
          alert("getUserMedia() failed - use https: address");
        }
      );
    }
  ...
  function paint(anitime) {
    ...
    // clean up the track
    for (
      var i = track.length - ctx.width, x = 0;
      i < track.length - 1;
      i++, x++
    ) {
      if (i >= 1 && track[i][0] > 0) {
        ...
        if (
          ...
          track[i][0] = 2 * f2;
        else if (f1 < 0.75 * f2 && f3 < 0.75 * f2) track[i][0] = 0;
        else if (f1 > 1.25 * f2 && f3 > 1.25 * f2) track[i][0] = 0;
      }
    }
    ...
    var y = Math.round(
      ctx.height -
        (ctx.height * (Math.log(warnOnMin) - logfxmin)) / (logfxmax - logfxmin)
    );
    ctx.moveTo(0, y);
    ...
    for (var f = 50; f <= fxmax; f += 50) {
      var y = Math.round(
        ctx.height -
          (ctx.height * (Math.log(f) - logfxmin)) / (logfxmax - logfxmin)
      );
      ctx.moveTo(0, y);
      ctx.lineTo(ctx.width, y);
    }
    ...
    var durationFormatted =
      days < 1
        ...
        : "Whoa girl. Time to take a break.";
    ...
    if (ampmod) {
      ...
      for (var i = offset; i < track.length - 1; i++) {
        if (i >= 0 && track[i][0] > 0) {
          ...
          var y1 =
            (ctx.height * (Math.log(track[i][0]) - logfxmin)) /
            (logfxmax - logfxmin);
          var y2 =
            (ctx.height * (Math.log(track[i + 1][0]) - logfxmin)) /
            (logfxmax - logfxmin);
          var w1 = (ctx.height * track[i][1]) / 10;
          var w2 = (ctx.height * track[i + 1][1]) / 10;
          var change = Math.abs(
            (2 * (track[i][0] - track[i + 1][0])) /
  ...
  function onconfigdlg() {
    ...
    if ($("scroll1").checked) hscroll = 1;
    else if ((hscroll = $("scroll2").checked)) hscroll = 2;
    else hscroll = 0;
